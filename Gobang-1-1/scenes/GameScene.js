...
GameScene.CheckForWin = function(che,tilesValues)
{
						for (var i = 0;i < 10;i++) {
							// Chess abscissa
							var x = che[i]%10;
							// Vertical coordinates of chess pieces
							var y = 10(che[i]/10);
							//\ Such a tilt judgment
							if ( x <= 10 - 5 && y <= 10 - 5 && che[i] != 0 ) {
								if( che[i+1*10+1] != 0 && che[i+2*10+2] != 0 && che[i+3*10+3] != 0 && che[i+4*10+4] != 0 ){
									alert(tilesValues+'Win!');
									// // Refresh the page after victory
									location.replace(location);
									return true;
								}
							};
							// | Such vertical judgement
							if ( y <= 10 - 5 && che[i] != 0 ) {
								if( che[i+1*10] != 0 && che[i+2*10] != 0 && che[i+3*10] != 0 && che[i+4*10] != 0 ){
									alert(tilesValues+'Win!');
									// Location object method replace () replaces the current document with a new document
									location.replace(location);
									return true;
								}
							};
